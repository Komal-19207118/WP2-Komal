                ...
                if (password_verify($password, $user['password'])){
                    ...
                    if($user['role_id'] == 1){
                        ...
                        if($user['image'] == 'default.jpg'){
                            $this->session->set_flashdata('pesan', '<div class="allert alert-info alert-messsage" role="alert">Silahkan Ubah Profile anda untuk ubah poto profil</div>');
                        }
                        redirect('user');

                    }
                }else{
                    $this->session->set_flashdata('pesan', '<div class="allert alert-info alert-messsage" role="alert">Password Salah</div>');
                    redirect('autentifikasi');
                }

            }else{
                $this->session->set_flashdata('pesan', '<div class="allert alert-info alert-messsage" role="alert">user tidak aktif</div>');
                redirect('autentifikasi');
            }
        }else{
            $this->session->set_flashdata('pesan', '<div class="allert alert-info alert-messsage" role="alert">email tidak terdaftar</div>');
            redirect('autentifikasi');
        }
    }
